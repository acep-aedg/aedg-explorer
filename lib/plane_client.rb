...
module PlaneClient
  ...
  def endpoint
    @endpoint ||= begin
      ...
      slug = ENV.fetch("PLANE_WORKSPACE_SLUG") { raise "Missing ENV['PLANE_WORKSPACE_SLUG']" }
      project = ENV.fetch("PLANE_PROJECT_ID") { raise "Missing ENV['PLANE_PROJECT_ID']" }

      "#{base_url}/api/v1/workspaces/#{slug}/projects/#{project}/work-items/"
    end
  end
