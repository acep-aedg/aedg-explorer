...
### api
Tools:
- automation/orchestration (dagster?)
- python for actual api scripts
- sha256sum for validity
- storage for stashing raw data (in repo for now, switch later)

Goals:
This step kicks off the pipeline, running scripts to pull data down from various APIs and stash it unchanged in a storage location. Output is CSV, GeoJSON, etc. 
